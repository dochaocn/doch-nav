...
/** sync 单键上限 8192 字节，按 UTF-8 切片，留出 JSON 序列化余量。 */
const CHUNK_MAX_UTF8_BYTES = 7500;
...
function _splitUtf8ByteChunks(str, maxBytes) {
...
async function _writeStashBundleToSync(bundle) {
  ...
  const chunks = _splitUtf8ByteChunks(raw, CHUNK_MAX_UTF8_BYTES);
  const meta = {
    version: 2,
...
export async function setStashState(state) {
  ...
  try {
    ...
    if (_isQuotaError(e)) {
      ...
      // 已成功降级到本机；再抛错会让工具栏误显红色感叹号，且新标签页横幅会提示仅本机。
      return;
    }
    throw e;
  }
...
function _isQuotaError(e) {
  ...
  return (
    m.includes("quota") ||
    m.includes("8192") ||
    m.includes("quota_bytes_per_item") ||
    (m.includes("maximum") && m.includes("item"))
  );
}
